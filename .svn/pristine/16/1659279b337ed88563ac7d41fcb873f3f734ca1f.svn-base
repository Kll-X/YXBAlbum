...
import {getTemplateList} from "../../lib/ajax";
import * as utils from "../../lib/utils";
import aniviewCom from './aniview.vue';
...
export default {
  ...
  methods: {
    ...
    getParams(pageNum){
        ...
        if(this.tabs[this.curIndex].label6){
            obj.label6 = this.tabs[this.curIndex].label6;
        }
        return obj
    },
    /* 联网加载列表数据
    ...
    getListDataFromNet(
      ...
      errorCallback
        ...
        let self = this;
        getTemplateList(self.getParams(pageNum)).then(function (data) {
                data = utils.processReturnObj(data);
                if(data.resultCode === 0){
                    //todo  临时模拟关闭模板收费功能
                    // if(data.data.templateList.length > 0){
                    //     data.data.templateList.forEach((item)=>{
                    //         item.price = 0
                    //     });
                    // }
