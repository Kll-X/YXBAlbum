      ...
      <swiper-slide v-for="(tab,i) in tabs" :key="i">
        ...
        >
          <div class="dataList" :id="'dataList'+i">
                <page-view  class="pages" :listData="tab.list"></page-view >
          </div>
        </mescroll-vue>
      </swiper-slide>
    </swiper>
...
import {getBgTemlates} from "../../../../lib/ajax";
  ...
  methods: {
    ...
    getListDataFromNet(
      ...
      errorCallback
        ...
        let self = this;
        getBgTemlates({
              "method": "c=upfile&a=syslist_all&fileType=1302&bizType=" + this.tabs[tabIndex].typeCode + "&pageSize=12&pageNo=" + pageNum,
              "methodType": 'get'
          }).then(function (data) {
                        ...
                        if (data.data.list.length > 0) {
                            let listData = [];
                                data.data.list.forEach(function (item, idx) {
                                    let bgSrc = item.tmbPath;
                                    let bgFullSrc = PREFIX_FILE + 'Uploads/' + item.tmbPath;
                                    listData.push({
                                        bgSrc: bgSrc,
                                        bgFullSrc: bgFullSrc
                                    });
